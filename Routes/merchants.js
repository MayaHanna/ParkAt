...
router.put("/points", async (req, res) => {
    try {
        const newMerchant = await addPointsToMerchant(req.body.userMail, req.body.pointsToAdd);
...
router.put("/", async (req, res) => {
    ...
    try {
        const newMerchantAfterEdit = await editMerchant(req.body.formData.userMail, newMerchant);
